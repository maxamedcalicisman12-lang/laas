...
exports.show = (req, res, next) => {
  try {
    ...
    item.customer = item.customer_id
      ? db.prepare('SELECT * FROM customers WHERE id = ?').get(item.customer_id)
      : null;
    item.buyer = item.sold_to
      ? db.prepare('SELECT * FROM customers WHERE id = ?').get(item.sold_to)
      : null;
    item.payments = db
      .prepare(
...
exports.store = (req, res, next) => {
  try {
    ...
    logActivity(req.user.id, 'created', 'used_item', `Created used item ${req.body.name}`);
...
exports.update = (req, res, next) => {
  try {
    ...
    logActivity(req.user.id, 'updated', 'used_item', `Updated used item ${req.body.name}`);
...
exports.toggleStatus = (req, res, next) => {
  try {
    ...
    db.prepare('UPDATE used_items SET status = ?, updated_at = ? WHERE id = ?').run(
      newStatus,
      db.now(),
      item.id
    );
    logActivity(req.user.id, 'updated', 'used_item', `Toggled status of used item ${item.name}`);
...
exports.destroy = (req, res, next) => {
  try {
    ...
    db.prepare('UPDATE used_items SET deleted_at = ? WHERE id = ?').run(db.now(), item.id);
    logActivity(req.user.id, 'deleted', 'used_item', `Deleted used item ${item.name}`);
