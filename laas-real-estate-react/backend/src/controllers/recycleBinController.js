...
exports.restore = (req, res, next) => {
  try {
    ...
    db.prepare(`UPDATE ${entry.table} SET deleted_at = NULL WHERE id = ?`).run(id);
    ...
    const displayName =
      ...
      `#${id}`;
    logActivity(req.user.id, 'restored', type, `Restored ${entry.label} ${displayName}`);
...
exports.forceDelete = (req, res, next) => {
  try {
    ...
    db.prepare(`DELETE FROM ${entry.table} WHERE id = ?`).run(id);
