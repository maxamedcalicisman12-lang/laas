...
exports.store = (req, res, next) => {
  try {
    ...
    logActivity(req.user.id, 'created', 'user', `Created user ${req.body.name}`);
...
exports.update = (req, res, next) => {
  try {
    ...
    logActivity(req.user.id, 'updated', 'user', `Updated user ${req.body.name}`);
...
exports.destroy = (req, res, next) => {
  try {
    ...
    db.prepare('UPDATE users SET deleted_at = ? WHERE id = ?').run(db.now(), user.id);
    logActivity(req.user.id, 'deleted', 'user', `Deleted user ${user.name}`);
