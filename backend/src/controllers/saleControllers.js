...
async function syncCommission(sourceType, sourceId, propertyId, customerId, amount, registeredBy) {
  const { rate, commission } = commissionFor(amount);
  await db.prepare('DELETE FROM commissions WHERE commission_type = ? AND commission_id = ?').run(
    sourceType,
    sourceId
  );
  await db
...
function makeSaleController({ table, module }) {
  ...
  async function show(req, res, next) {
    try {
      ...
      row.property = await db.prepare('SELECT * FROM properties WHERE id = ?').get(row.property_id);
      row.customer = await db.prepare('SELECT * FROM customers WHERE id = ?').get(row.customer_id);
      row.payments = await db
        .prepare(
  ...
  async function createMeta(req, res, next) {
    try {
      ...
      if (isLand) {
        properties = await db
          .prepare("SELECT * FROM properties WHERE type IN ('land','land-sale') AND status != 'sold' AND deleted_at IS NULL")
          .all();
      } else {
        const propTypes = isRental
          ? "('house-rental')"
          : "('house','apartment','villa','house-sale')";
        properties = await db
          .prepare(`SELECT * FROM properties WHERE type IN ${propTypes} AND status != ? AND deleted_at IS NULL`)
  ...
  async function editMeta(req, res, next) {
    try {
      const row = await db
        .prepare(`SELECT * FROM ${table} WHERE id = ? AND deleted_at IS NULL`)
        .get(req.params.id);
      ...
      const properties = isLand
        ? await db.prepare("SELECT * FROM properties WHERE type IN ('land','land-sale') AND deleted_at IS NULL").all()
        : await db
            .prepare(
              `SELECT * FROM properties WHERE type IN ${isRental ? "('house-rental')" : "('house','apartment','villa','house-sale')"} AND deleted_at IS NULL`
            )
            .all();
      const customers = await db
        .prepare('SELECT * FROM customers WHERE deleted_at IS NULL ORDER BY first_name')
  ...
  async function update(req, res, next) {
    try {
      ...
      await db.prepare(`UPDATE ${table} SET ${sets.join(', ')} WHERE id = ?`).run(...values);
      ...
      await logActivity(req.user.id, 'updated', module, `Updated ${label} #${row.id}`);
  ...
  async function toggleStatus(req, res, next) {
    try {
      ...
      await db.prepare(`UPDATE ${table} SET status = ?, updated_at = ? WHERE id = ?`).run(
        newStatus,
        db.now(),
        row.id
      );
      await logActivity(req.user.id, 'updated', module, `Toggled status of ${label} #${row.id}`);
  ...
  async function destroy(req, res, next) {
    try {
      ...
      await db.prepare(`UPDATE ${table} SET deleted_at = ? WHERE id = ?`).run(db.now(), row.id);
      await logActivity(req.user.id, 'deleted', module, `Deleted ${label} #${row.id}`);
