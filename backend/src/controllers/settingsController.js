...
exports.toggleTwoFactor = async (req, res, next) => {
  try {
    ...
    await db.prepare('UPDATE users SET two_factor_enabled = ?, updated_at = ? WHERE id = ?').run(
      newValue,
      db.now(),
      req.user.id
    );
...
exports.updateSecurity = async (req, res, next) => {
  try {
    ...
    await settings.setValue('password_min_length', req.body.password_min_length);
    await settings.setValue('password_require_uppercase', boolify(req.body.password_require_uppercase) ? '1' : '0');
    await settings.setValue('password_require_numeric', boolify(req.body.password_require_numeric) ? '1' : '0');
    await settings.setValue('password_require_special', boolify(req.body.password_require_special) ? '1' : '0');
    await settings.setValue('login_max_attempts', req.body.login_max_attempts);
    await settings.setValue('login_lockout_minutes', req.body.login_lockout_minutes);
...
exports.changePassword = async (req, res, next) => {
  try {
    ...
    await db.prepare('UPDATE users SET password = ?, updated_at = ? WHERE id = ?').run(
...
exports.updateProfilePicture = async (req, res, next) => {
  try {
    ...
    await db.prepare('UPDATE users SET profile_picture = ?, updated_at = ? WHERE id = ?').run(
      picturePath,
      db.now(),
      req.user.id
    );
...
exports.removeProfilePicture = async (req, res, next) => {
  try {
    ...
    await db.prepare('UPDATE users SET profile_picture = NULL, updated_at = ? WHERE id = ?').run(
      db.now(),
      req.user.id
    );
...
exports.updateColor = async (req, res, next) => {
  try {
    await validate(req.body, { accent_color: 'required|string|max:50' });
    await settings.setValue('accent_color', req.body.accent_color);
...
exports.updateFont = async (req, res, next) => {
  try {
    ...
    await settings.setValue('font_size', req.body.font_size);
    await settings.setValue('font_family', req.body.font_family);
...
exports.updateCommission = async (req, res, next) => {
  try {
    ...
    await settings.setValue('commission_rate', rate);
...
exports.clearData = async (req, res, next) => {
  try {
    await validate(req.body, { table: `required|in:${CLEAR_TABLES.join(',')}` });
    await db.prepare(`DELETE FROM ${req.body.table}`).run();
