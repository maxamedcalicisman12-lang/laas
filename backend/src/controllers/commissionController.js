...
exports.show = async (req, res, next) => {
  try {
    ...
    row.property = row.property_id
      ? await db.prepare('SELECT * FROM properties WHERE id = ?').get(row.property_id)
      : null;
    row.customer = row.customer_id
      ? await db.prepare('SELECT * FROM customers WHERE id = ?').get(row.customer_id)
      : null;
    row.registered_by_user = row.registered_by
      ? await db.prepare('SELECT name FROM users WHERE id = ?').get(row.registered_by)
      : null;
...
exports.destroy = async (req, res, next) => {
  try {
    ...
    await db.prepare('UPDATE commissions SET deleted_at = ? WHERE id = ?').run(db.now(), row.id);
    await logActivity(req.user.id, 'deleted', 'commission', `Deleted commission #${row.id}`);
