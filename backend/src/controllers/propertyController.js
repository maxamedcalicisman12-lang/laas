...
const TYPE_ALIASES = { 'land-sale': 'land' };
    ...
    p.payments = await db
      .prepare(
        `SELECT payments.*, customers.first_name || ' ' || customers.last_name AS customer_name
         ...
         LEFT JOIN land_sales ON payments.payable_type = 'App\\Models\\LandSale' AND payments.payable_id = land_sales.id
         LEFT JOIN house_sales ON payments.payable_type = 'App\\Models\\HouseSale' AND payments.payable_id = house_sales.id
         LEFT JOIN house_rentals ON payments.payable_type = 'App\\Models\\HouseRental' AND payments.payable_id = house_rentals.id
         WHERE (land_sales.property_id = ? OR house_sales.property_id = ? OR house_rentals.property_id = ?)
...
exports.store = async (req, res, next) => {
  try {
    ...
    const property = await db.prepare('SELECT * FROM properties WHERE id = ?').get(info.lastInsertRowid);
    await logActivity(
      req.user.id,
      'created',
      'property',
...
exports.update = async (req, res, next) => {
  try {
    ...
    await logActivity(req.user.id, 'updated', 'property', `Updated property ${req.body.title}`);
...
exports.destroy = async (req, res, next) => {
  try {
    ...
    await db.prepare('UPDATE properties SET deleted_at = ? WHERE id = ?').run(db.now(), property.id);
    await logActivity(req.user.id, 'deleted', 'property', `Deleted property ${property.title}`);
