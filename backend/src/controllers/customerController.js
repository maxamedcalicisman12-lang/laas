...
exports.show = async (req, res, next) => {
  try {
    ...
    customer.land_sales = await db
      ...
      .all(customer.id);
    customer.house_rentals = await db
      .prepare(
    ...
    const payableMap = {
      'App\\Models\\LandSale': 'land_sales',
      'App\\Models\\HouseSale': 'house_sales',
      'App\\Models\\HouseRental': 'house_rentals',
      'App\\Models\\UsedItem': 'used_items',
    };
    for (const pay of customer.payments) {
      const table = payableMap[pay.payable_type];
      if (!table || !pay.payable_id) continue;
      const rel = await db
        .prepare(`SELECT property_id FROM ${table} WHERE id = ? AND deleted_at IS NULL`)
        .get(pay.payable_id);
      if (rel && rel.property_id) {
        const prop = await db.prepare('SELECT title FROM properties WHERE id = ?').get(rel.property_id);
        if (prop) pay.__property_title = prop.title;
      }
    }
...
exports.store = async (req, res, next) => {
  try {
    ...
    const customer = await db.prepare('SELECT * FROM customers WHERE id = ?').get(info.lastInsertRowid);
    await logActivity(
      req.user.id,
      'created',
      'customer',
...
exports.update = async (req, res, next) => {
  try {
    ...
    await validate(req.body, rules(customer.id));
...
exports.destroy = async (req, res, next) => {
  try {
    ...
    await db.prepare('UPDATE customers SET deleted_at = ? WHERE id = ?').run(db.now(), customer.id);
    await logActivity(
      req.user.id,
      'deleted',
      'customer',
