...
exports.index = async (req, res, next) => {
  try {
    ...
    const total = (await db.prepare(`SELECT COUNT(*) as c FROM (${sql})`).get(req.user.id)).c;
    const page = Math.max(1, parseInt(req.query.page, 10) || 1);
    const rows = await db.prepare(`${sql} LIMIT 20 OFFSET ?`).all(req.user.id, (page - 1) * 20);
    rows.forEach((n) => {
      try {
...
exports.markAsRead = async (req, res, next) => {
  try {
    ...
    await db.prepare('UPDATE notifications SET read_at = ?, updated_at = ? WHERE id = ?').run(
      db.now(),
      db.now(),
      notification.id
    );
